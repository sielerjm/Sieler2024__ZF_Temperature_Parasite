...
data.dynamics[["Alpha"]] <- list()

data.dynamics[["Alpha"]][["Metadata"]] <- list()
data.dynamics[["Alpha"]][["Displacement"]] <- list()
data.dynamics[["Alpha"]][["Volatility"]] <- list()
data.dynamics[["Alpha"]][["Velocity"]] <- list()
data.dynamics[["Alpha"]][["Acceleration"]] <- list()

data.dynamics[["Beta"]] <- list()

data.dynamics[["Beta"]][["Metadata"]] <- list()
data.dynamics[["Beta"]][["Displacement"]] <- list()
data.dynamics[["Beta"]][["Volatility"]] <- list()
data.dynamics[["Beta"]][["Velocity"]] <- list()
data.dynamics[["Beta"]][["Acceleration"]] <- list()
...
data.dynamics[["Alpha"]][["Displacement"]]$median_diversity_initial_28C <-
  data.dynamics[["Alpha"]][["Metadata"]] %>%
...
data.dynamics[["Alpha"]][["Displacement"]]$init28 <-
  data.dynamics[["Alpha"]][["Metadata"]] %>%
  
  # Merge dataframe with previously calculated median scores to data frame with diversity scores and metadata
  dplyr::left_join(data.dynamics[["Alpha"]][["Displacement"]]$median_diversity_initial_28C, by = "Alpha.Metric") %>%
...
# Step 1: Filter initial samples at 28°C and calculate their median diversity score
data.dynamics[["Beta"]][["Displacement"]]$median_diversity_initial_28C <-
  data.dynamics[["Beta"]][["Metadata"]] %>%
  dplyr::ungroup() %>%
  dplyr::filter(DPE == 0, Temperature == 28) %>%
...
# Step 2: Calculate displacement for each sample
data.dynamics[["Beta"]][["Displacement"]]$init28 <-
  data.dynamics[["Beta"]][["Metadata"]] %>%
  dplyr::left_join(data.dynamics[["Beta"]][["Displacement"]]$median_diversity_initial_28C, by = "Beta.Metric") %>% ## Multiple metrics
  dplyr::group_by(Beta.Metric) %>%
...
# Step 1: Filter initial samples at 28°C and calculate their median diversity score
data.dynamics[["Alpha"]][["Displacement"]]$median_diversity_relTime_28C <-
  data.dynamics[["Alpha"]][["Metadata"]] %>%
...
# Step 2: Calculate displacement for each sample
data.dynamics[["Alpha"]][["Displacement"]]$relTime28 <-
  data.dynamics[["Alpha"]][["Metadata"]] %>%
  
  # Merge data frame from previous step to data frame
  dplyr::left_join(data.dynamics[["Alpha"]][["Displacement"]]$median_diversity_relTime_28C, by = c("Alpha.Metric", "DPE")) %>%
...
# Step 1: Filter initial samples at 28°C and calculate their median diversity score
data.dynamics[["Beta"]][["Displacement"]]$median_diversity_relTime_28C <-
  data.dynamics[["Beta"]][["Metadata"]] %>%
  dplyr::filter(Treatment == "Control", Temperature == 28) %>%
...
# Step 2: Calculate displacement for each sample
data.dynamics[["Beta"]][["Displacement"]]$relTime28 <-
  data.dynamics[["Beta"]][["Metadata"]] %>%
  dplyr::left_join(data.dynamics[["Beta"]][["Displacement"]]$median_diversity_relTime_28C, by = c("Beta.Metric", "DPE")) %>%
  dplyr::mutate(Displacement = Beta.Score_norm - Ref.Score) %>%
...
# Step 1: Filter initial samples at 28°C and calculate their median diversity score
data.dynamics[["Alpha"]][["Displacement"]]$median_diversity_relTimeTemp <-
  data.dynamics[["Alpha"]][["Metadata"]] %>%
...
# Step 2: Calculate displacement for each sample
data.dynamics[["Alpha"]][["Displacement"]]$relTimeTemp <-
  data.dynamics[["Alpha"]][["Metadata"]] %>%
  
  # Merge data frame from previous step to data frame
  dplyr::left_join(data.dynamics[["Alpha"]][["Displacement"]]$median_diversity_relTimeTemp, by = c("Alpha.Metric", "DPE", "Temperature")) %>%
...
# Step 1: Filter initial samples at 28°C and calculate their median diversity score
data.dynamics[["Beta"]][["Displacement"]]$median_diversity_relTimeTemp <-
  data.dynamics[["Beta"]][["Metadata"]] %>%
  dplyr::filter(Treatment == "Control") %>%
...
# Step 2: Calculate displacement for each sample
data.dynamics[["Beta"]][["Displacement"]]$relTimeTemp <-
  data.dynamics[["Beta"]][["Metadata"]] %>%
  dplyr::left_join(data.dynamics[["Beta"]][["Displacement"]]$median_diversity_relTimeTemp, by = c("Beta.Metric", "DPE", "Temperature")) %>%
  dplyr::mutate(Displacement = Beta.Score_norm - Ref.Score) %>%
  dplyr::select(Sample, DPE, Treatment, Temperature, Tank.ID, Beta.Metric, Beta.Score_norm,Ref.Score, Displacement) %>%
  dplyr::ungroup() %>% 
  dplyr::arrange(DPE, Treatment, Temperature, Tank.ID)
...
data.dynamics[["Alpha"]][["Velocity"]]$median_displacement_by_group <- data.dynamics[["Alpha"]][["Displacement"]]$relTime28 %>%
...
data.dynamics[["Alpha"]][["Velocity"]]$shifted_median_displacement <- data.dynamics[["Alpha"]][["Velocity"]]$median_displacement_by_group %>%
...
data.dynamics[["Alpha"]][["Velocity"]]$data.vel <- data.dynamics[["Alpha"]][["Displacement"]]$relTime28 %>%
  # dplyr::mutate(DPE = as.numeric(levels(DPE)[DPE])) %>%
  dplyr::full_join(data.dynamics[["Alpha"]][["Velocity"]]$shifted_median_displacement, by = c("Alpha.Metric", "Treatment", "Temperature", "DPE")) %>%
...
data.dynamics[["Alpha"]][["Velocity"]]$median_velocity_by_group <- data.dynamics[["Alpha"]][["Velocity"]]$data.vel %>%
...
data.dynamics[["Alpha"]][["Acceleration"]]$shifted_median_velocity <- data.dynamics[["Alpha"]][["Velocity"]]$median_velocity_by_group %>%
  dplyr::arrange(Treatment, Temperature, DPE) %>%
  dplyr::group_by(Alpha.Metric, Treatment, Temperature) %>%
...
data.dynamics[["Alpha"]][["Acceleration"]]$data.acc <- data.dynamics[["Alpha"]][["Velocity"]]$data.vel %>%
  dplyr::full_join(data.dynamics[["Alpha"]][["Velocity"]]$shifted_median_displacement, by = c("Alpha.Metric", "Treatment", "Temperature", "DPE")) %>%
  ...
  dplyr::full_join(data.dynamics[["Alpha"]][["Acceleration"]]$shifted_median_velocity, by = c("Alpha.Metric", "Treatment", "Temperature", "DPE")) %>%
...
# Step 1: Calculate median diversity score for each Treatment, Temperature, and DPE
data.dynamics[["Beta"]][["Velocity"]]$median_displacement_by_group <- data.dynamics[["Beta"]][["Displacement"]]$relTime28 %>%
  # dplyr::mutate(DPE = as.numeric(levels(DPE)[DPE])) %>%
...
# Step 2: Create a shifted version of median diversity to align with the next DPE
data.dynamics[["Beta"]][["Velocity"]]$shifted_median_displacement <- data.dynamics[["Beta"]][["Velocity"]]$median_displacement_by_group %>%
  dplyr::arrange(Treatment, Temperature, DPE) %>%
...
# Step 3: Calculate the change in diversity score for each sample
data.dynamics[["Beta"]][["Velocity"]]$data.vel <- data.dynamics[["Beta"]][["Displacement"]]$relTime28 %>%
  # dplyr::mutate(DPE = as.numeric(levels(DPE)[DPE])) %>%
  dplyr::full_join(data.dynamics[["Beta"]][["Velocity"]]$shifted_median_displacement, by = c("Beta.Metric", "Treatment", "Temperature", "DPE")) %>%
...
# Step 4: Calculate median diversity score for each Treatment, Temperature, and DPE
data.dynamics[["Beta"]][["Velocity"]]$median_velocity_by_group <- data.dynamics[["Beta"]][["Velocity"]]$data.vel %>%
  dplyr::filter(!is.na(Velocity)) %>%
  dplyr::group_by(Beta.Metric, Treatment, Temperature, DPE) %>%
  dplyr::summarize(median_velocity = median(Velocity), .groups = 'drop')

# Step 5: Create a shifted version of median diversity to align with the next DPE
data.dynamics[["Beta"]][["Acceleration"]]$shifted_median_velocity <- data.dynamics[["Beta"]][["Velocity"]]$median_velocity_by_group %>%
  dplyr::arrange(Treatment, Temperature, DPE) %>%
  dplyr::group_by(Beta.Metric, Treatment, Temperature) %>%
...
# Step 6: Calculate the change in diversity score for each sample
data.dynamics[["Beta"]][["Acceleration"]]$data.acc <- data.dynamics[["Beta"]][["Velocity"]]$data.vel %>%
  dplyr::full_join(data.dynamics[["Beta"]][["Velocity"]]$shifted_median_displacement, by = c("Beta.Metric", "Treatment", "Temperature", "DPE")) %>%
  dplyr::select(-previous_timepoint) %>%
  ...
  dplyr::full_join(data.dynamics[["Beta"]][["Acceleration"]]$shifted_median_velocity, by = c("Beta.Metric", "Treatment", "Temperature", "DPE")) %>%
...
# Step 1: Calculate median diversity score for each Treatment, Temperature, and DPE
data.dynamics[["Beta"]][["Velocity"]]$median_displacement_by_group <- data.dynamics[["Beta"]][["Displacement"]]$relTimeTemp %>%
  # dplyr::mutate(DPE = as.numeric(levels(DPE)[DPE])) %>%
  dplyr::group_by(Beta.Metric, Treatment, Temperature, DPE) %>%
  dplyr::summarize(median_displacement = median(Displacement), .groups = 'drop') %>%
  dplyr::ungroup()
...
# Step 3: Calculate the change in diversity score for each sample
data.dynamics[["Beta"]][["Velocity"]]$data.vel <- data.dynamics[["Beta"]][["Displacement"]]$relTimeTemp %>%
  # dplyr::mutate(DPE = as.numeric(levels(DPE)[DPE])) %>%
  dplyr::full_join(data.dynamics[["Beta"]][["Velocity"]]$shifted_median_displacement, by = c("Beta.Metric", "Treatment", "Temperature", "DPE")) %>%
  dplyr::relocate(c("Sample", "Treatment", "Temperature", "DPE", "Tank.ID", "Beta.Metric", "Beta.Score_norm", "median_displacement", "previous_median_displacement", "previous_timepoint"), .before = 1) %>%
  dplyr::ungroup() %>%
...
data.dynamics[["Alpha"]][["Volatility"]]$All <- 
  data.dynamics[["Alpha"]][["Displacement"]]$relTime28 %>%
  # dplyr::mutate(DPE = as.numeric(levels(DPE)[DPE])) %>%
  dplyr::full_join(data.dynamics[["Alpha"]][["Velocity"]]$data.vel) %>%
  dplyr::full_join(data.dynamics[["Alpha"]][["Acceleration"]]$data.acc) %>%
  dplyr::group_by(Alpha.Metric, Treatment, Temperature, DPE) %>%
...
data.dynamics[["Beta"]][["Volatility"]]$All <- 
  data.dynamics[["Beta"]][["Displacement"]]$relTime28 %>%
  # dplyr::mutate(DPE = as.numeric(levels(DPE)[DPE])) %>%
  dplyr::full_join(data.dynamics[["Beta"]][["Velocity"]]$data.vel) %>%
  dplyr::full_join(data.dynamics[["Beta"]][["Acceleration"]]$data.acc) %>%
  dplyr::group_by(Beta.Metric, Treatment, Temperature, DPE) %>%
  dplyr::mutate(Volatility = sd(Displacement), .after = "Displacement") %>%
  dplyr::ungroup() 
